...
const IndustryPage = () => {
  ...
  React.useEffect(() => {
    const loadIndustryPage = async () => {
      if (slug) {
        try {
          const savedContent = await loadPage(slug, 'industry');
          if (savedContent) {
            const parsedPage = parsePage(`${slug}.md`, savedContent, 'industry');
            setPage(parsedPage);
          }
        } catch (error) {
  ...
  if (!page) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="py-20 flex-1">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h1 className="text-4xl font-light text-slate-900 mb-6">Industry page not found</h1>
            <p className="text-xl text-slate-600 mb-8 font-light">The industry page you're looking for doesn't exist or has been moved.</p>
            <BackButton onClick={() => navigate('/industries')}>
              Back to Industries
            </BackButton>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <EditButton 
                variant="outline"
                onClick={() => navigate(`/industries/edit/${page.slug}`)}
